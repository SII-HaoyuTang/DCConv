...
class AggregationLayer:
    ...
    def __init__(self):
        self.cache: tuple[npt.NDArray, npt.NDArray] = None

    def forward(self, features: npt.NDArray, weights: npt.NDArray) -> npt.NDArray:
        """
        前向传播 (Forward Pass).

        Args:
            features (npt.NDArray): 邻域特征矩阵。
                Shape: (Ci, N, n)
            weights (npt.NDArray): 动态生成的卷积核权重。
                Shape: (Co, Ci, N, n)

        Returns:
            output (npt.NDArray): 聚合后的特征。
                Shape: (Co, N)
        """
        ...
        output = np.einsum("cuk, ocuk -> ou", features, weights)

        return output

    def backward(self, grad_output: npt.NDArray) -> tuple[npt.NDArray, npt.NDArray]:
        r"""
        反向传播 (Backward Pass).
        ...
        Args:
            grad_output (npt.NDArray): 来自下一层的梯度 $\partial L / \partial Y$。
                Shape: (Co, N)

        Returns:
            grad_features (npt.NDArray): 对输入特征的梯度。Shape: (Ci, N, n)
            grad_weights (npt.NDArray): 对权重的梯度。Shape: (Co, Ci, N, n)
        """
        ...
        #    Einsum: 'ou, ocuk -> cuk'
        # -----------------------------------------------------------
        grad_features = np.einsum("ou, ocuk -> cuk", grad_output, weights)
        ...
        #    Einsum: 'ou, cuk -> ocuk'
        # -----------------------------------------------------------
        grad_weights = np.einsum("ou, cuk -> ocuk", grad_output, features)
