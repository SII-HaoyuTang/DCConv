...
def calc_pairwise_distance(coords: npt.NDArray) -> npt.NDArray:
    r"""
    ...
    Mathematical Formulation:
        ...
        代码对应关系:
        - `square_sum`: $||x||^2$ (N, 1)
        - `np.dot(coords, coords.T)`: $X \cdot X^T$ 即点积项 (N, N)

    Note:
        1. **数值稳定性**: 由于浮点数精度误差，$a^2 + b^2 - 2ab$ 可能会产生极小的负数（如 -1e-8）。
           因此必须使用 `np.maximum(..., 0)` 进行截断，否则 `np.sqrt` 会产生 NaN。
        2. **对角线**: 理论上对角线元素应该严格为 0，但计算结果可能是极小的正数或 0。
    """
    square_sum = np.sum(coords**2, axis=1, keepdims=True)
    dist_sq = square_sum + square_sum.T - 2 * np.dot(coords, coords.T)
...
class BaseSelector(ABC):
    ...
    @abstractmethod
    def select(self, coords: npt.NDArray) -> npt.NDArray:
        pass

    def __call__(self, coords: npt.NDArray) -> npt.NDArray[np.int32]:
...
class KNNSelector(BaseSelector):
    ...
    def select(self, coords: npt.NDArray) -> npt.NDArray[np.int32]:
        """
        执行 KNN 选点计算。

        使用广播机制计算全对全 (All-to-All) 距离矩阵，并通过 argpartition
        快速提取前 k 个最近点的索引。

        Args:
            coords (np.ndarray): 输入点云坐标矩阵。
                Shape: (N, 3), 其中 N 是点的数量，3 代表 (x, y, z)。
                Dtype: 通常为 float32 或 float64。

        Returns:
            np.ndarray: 选中的邻居索引矩阵。
                Shape: (N, n_sample).
                Dtype: int (整数索引).
                Values: 范围在 [0, N-1] 之间。
        ...
        unsorted_indices = np.argpartition(dist_mat, self.n_sample, axis=1)
        knn_indices = unsorted_indices[:, : self.n_sample]
        return knn_indices.astype(np.int32)
...
class BallQuerySelector(BaseSelector):
    ...
    def select(self, coords: npt.NDArray) -> npt.NDArray[np.int32]:
        ...
        N = coords.shape[0]
        dist_mat = calc_pairwise_distance(coords)
        indices = np.zeros((N, self.n_sample), dtype=np.int32)

        for i in range(N):
            candidates = np.where(dist_mat[i] < self.radius)[0]
            k = len(candidates)
            ...
            if k == 0:
                ...
                sorted_local_idx = np.argsort(valid_dist)
...
class DilatedKNNSelector(BaseSelector):
    ...
    def select(self, coords: npt.NDArray) -> npt.NDArray[np.int32]:
        ...
        Logic:
            ...
            2. 采样：使用 Python 切片 `[::dilation]` 进行稀疏采样。
        ...
        if dilated_indices.shape[1] < self.n_sample:
            return sorted_indices[:, : self.n_sample]

        return dilated_indices.astype(np.int32)
