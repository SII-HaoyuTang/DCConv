...
def benchmark_ablation():
    ...
    with open(csv_filename, mode="w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Method", "N", "Avg Latency (ms)"])
    ...
    for N in N_list:
        ...
        print(f"PCA Overhead: {diff * 1000:.2f} ms per call")

        # Save to CSV
        with open(csv_filename, mode="a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["With PCA", N, avg_pca * 1000])
            writer.writerow(["No PCA", N, avg_no_pca * 1000])
